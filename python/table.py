...
class Table(BaseModel):
	...
	player_index: int = Field(description="The player who is next.")
	game_mode: Vorbehalt = Field(description="The Vorbehalt which is played this round.")
	cards_layed: list = Field(description="The cards already layed in this round.")
	...
	def lay_card(self, player, card):
		...
		if not self.__check_correct_bedient(player, card):
	...
	def __init__(self, start_player_index:int, game_mode:Vorbehalt):
		self.player_index = start_player_index
		self.game_mode = game_mode
		self.cards_layed = {i: None for i in range(4)}
	
	def __check_correct_bedient(self, player, card):
		return True # TODO